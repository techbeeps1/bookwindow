...
export default function AuthForm({ mode }: AuthFormProps) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (tab === "register" && mode === "login") {
        router.replace("/sign-up");
      } else if (tab === "login" && mode === "register") {
  ...
  async function handleSubmit1(event: FormEvent<HTMLFormElement>) {
    ...
    if (first_name.length > 255) {
      toast.error("First name cannot be more than 255 characters.");
      return;
    }
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 text-xs font-semibold bg-neutral-100 border border-neutral-200 rounded-full hover:bg-neutral-200 active:scale-95 transition-all focus:outline-none text-black"
                  >
                    Refresh
                  </button>
                </div>
