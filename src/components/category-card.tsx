...
function CategoryCard({
  ...
  return (
    <Link href={`/category/${cat_slug}`} className="group block w-full cursor-pointer">
      ...
      <h3 className="text-center lg:break-keep break-all mt-3 font-semibold text-gray-800 text-sm md:text-[15px] group-hover:text-black transition-colors duration-300">
        {cat_title}
      </h3>
    </Link>
  );
